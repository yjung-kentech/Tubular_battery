...
contourf(D_out_vec, C_rate_vec, data.T_max, 50, 'LineColor', 'none');
clim([min(data.T_max(:)) 100]);
hold on;
...
h = colorbar;
ylabel(h, 'T_{max} (^oC)', 'FontWeight', 'bold');
titleHandle = title('Maximum Temperature (^oC)');
