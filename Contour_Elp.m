...
contourf(D_out_vec, C_rate_vec, data.Elp_min, 15, 'LineColor', 'none');
clim([min(data.Elp_min(:)) max(data.Elp_min(:))]);
hold on;
...
h = colorbar;
ylabel(h, 'E_{lp} (V)', 'FontWeight', 'bold');
titleHandle = title('Lithium plating potential (V)');
